...
## Changelog
- `v1.68` — 지급자재 재고내역서 영역에 전용 업체 선택을 배치해 기록 입력용 거래처 선택과 분리.
  자재 보유 업체가 하나뿐이면 자동 선택하고, 업체 미선택 시 비활성 버튼 대신 안내 메시지를 표시한다.
  iOS에서 미리보기 레이어가 화면 밖에 있거나 배경과 함께 스크롤되지 않도록 동적 화면 높이·안전 영역·스크롤 잠금을 보강.
