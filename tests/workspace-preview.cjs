...
http.createServer((req,res)=>{
 ...
 if(!['index.html','v142-dutch-pay.css','workspace-system.css','workspace-layout.js'].includes(name)){res.writeHead(404);return res.end();}
 let content=fs.readFileSync(path.join(root,name),'utf8');
 if(name==='index.html')content=content.replace(/init\(\);\s*<\/script>/,fixture+'</script>');
